...
void processDmaSignalPWM() {
	...
	if (pwm[2] == 7) {
		...
		directionValidator++;
		if (directionValidator > 10) {
			directionValidator = 10;
...
void detechFrequency() {
	...
	if (signalValue[0] >= 100 && signalValue[0] < 200 && signalValue[1] > 0
			&& signalValue[1] < 5)
...
void setup() {
	...
	HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_2, (uint32_t*) dmaBuffer,
			dmaPulse);
	HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_3, (uint32_t*) dmaBuffer,
			dmaPulse);
	HAL_TIM_PWM_Start_DMA(&htim3, TIM_CHANNEL_4, (uint32_t*) dmaBuffer,
			dmaPulse);
...
void maincpp() {
	...
	uint8_t testMotor = 0;
	uint8_t motorSpinning = 0;
	while (1) {

		eRPM = (((1000000 / (commutationCNT / 32)) * 60) / 7); ///6; /6 will give you RPM
		uint8_t testCounter = 0;

		if ((REV_GPIO_Port->IDR & REV_Pin) == 0) { // switch direction when button pressed.
		...
		if (newDmaSignal || testMotor) {
			...
			if (adcValue > 0) {
				//											rb/rt
				batteryVoltages = adcValue * (3.3 / 4095.0)
						/ (10000.0 / 110000.0);
			...
			if (motorSpeed != motorSpeedCurrent) {
				if (motorSpeedCurrent > 200 && eRPM < 5000) { // TODO: add proper calculation to RPM.
					motorSpeedCurrent = 0;
					setDutyCycle(motorSpeedCurrent);
					HAL_Delay(20);
				...
				if (motorSpeedCurrent < 100) {
					...
					if ((motorSpeed - motorSpeedCurrent) > 50)
						motorSpeedCurrent += 50;
					else
						motorSpeedCurrent = motorSpeed;
				}
				...
				if (motorSpeedCurrent < 150) {
					checkRising = 1400
							- map(batteryVoltages, 8, 21, 1,
									map(motorSpeedCurrent, 1, 200, 2, 1400)

											);
				} else {
					checkRising = map(1024 - motorSpeedCurrent, 0, 1024, 1,
							320);
				}
				checkFalling = map(1024 - motorSpeedCurrent, 0, 1024, 1, 900);
			...
			if (frequencyType == 1) // PWM
					{
				TIM1->CNT = 0;
				HAL_TIM_IC_Start_DMA(&htim1, TIM_CHANNEL_3,
						(uint32_t*) dmaSignal, 3);
			} else {
				TIM1->CNT = 0;
				HAL_TIM_IC_Start_DMA(&htim1, TIM_CHANNEL_3,
						(uint32_t*) dmaSignal, 16);
			}
